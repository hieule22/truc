...
TEST_F(CodeGenerationTest, ParseBasicStatement) {
  ...
  MatchOutput("program foo; a: int; begin print a > 1; end;",
              ...
              "\t\tmove R0, a\n"
              "\t\tsub R0, #1\n"
              "\t\tbrne R0, _compare_false0\n"
              "\t\tbrez R0, _compare_false0\n"
              "\t\tmove R0, #1\n"
              "\t\tbrun _compare_done1\n"
              "_compare_false0:\n"
              "\t\tmove R0, #0\n"
...
TEST_F(CodeGenerationTest, IfStatement) {
  ...
  MatchOutput("program foo; n, p, q, r, t, v: int; s: bool;"
              ...
              "\t\tmove R1, n\n"
              "\t\tsub R1, R0\n"
              "\t\tbrne R1, _compare_false0\n"
              "\t\tbrez R1, _compare_false0\n"
              "\t\tmove R1, #1\n"
              "\t\tbrun _compare_done1\n"
              "_compare_false0:\n"
              "\t\tmove R1, #0\n"
  ...
  MatchOutput("program foo;"
              "begin if 0 = 0 then "
              "begin if 0 = 1 then "
              "begin print 10; end; end; end;",
  ...
  MatchOutput("program foo; begin "
              "if 0 = 0 then begin print 0; end "
              "else begin if 1 = 1 then begin print 1; end "
              "else begin if 2 = 2 then begin print 2; end; end; end; end;",
              ...
              "\t\tmove R0, #2\n"
              "\t\tsub R0, #2\n"
              "\t\tbrne R0, _compare_false8\n"
              "\t\tbrpo R0, _compare_false8\n"
              "\t\tmove R0, #1\n"
              "\t\tbrun _compare_done9\n"
              "_compare_false8:\n"
              "\t\tmove R0, #0\n"
...
TEST_F(CodeGenerationTest, Expression) {
  ...
  MatchOutput("program foo; a, b, c, d: bool; begin "
              ...
              "\t\tmove R2, #10\n"
              "\t\tsub R2, #12\n"
              "\t\tbrne R2, _compare_false0\n"
              "\t\tbrpo R2, _compare_false0\n"
              "\t\tmove R2, #1\n"
              "\t\tbrun _compare_done1\n"
              "_compare_false0:\n"
              "\t\tmove R2, #0\n"  // Compute 10 = 12 in R2.
